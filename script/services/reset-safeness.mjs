...
export function verifyResetSafeness(simpleModel, source, sink) {
  // ── Prerequisite: reset-safeness is only defined for impedance-free RDLTs ──
  // (Doñoz 2024, Definition 2.5.6; Malinao & Juayong 2024, Definition 10;
  //  Afable 2025, Theorem 3.2.4 — reset-safe ⟹ impedance-free by contradiction)
  const ifResult = verifyImpedanceFreeness(simpleModel, source, sink);
  const isImpedanceFree =
    ifResult?.instances?.[0]?.evaluation?.conclusion?.pass ?? false;
      ...
      instances: [
        {
          ...
          evaluation: {
            conclusion: {
              ...
              description:
                "Reset-safeness requires impedance-freeness as a prerequisite " +
                "(Doñoz 2024, Definition 2.5.6). This RDLT is not impedance-free, " +
                "therefore it cannot be reset-safe.",
            },
            criteria: [
              {
                pass: false,
                description:
                  "Prerequisite check failed: RDLT is not impedance-free. " +
                  "Reset-safeness is undefined for non-impedance-free RDLTs.",
              },
            ],
