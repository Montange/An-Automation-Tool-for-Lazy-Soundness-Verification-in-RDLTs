...
export class Soundness {
    ...
    */
    ...
    static checkLazySound(graph, evsa) {
        ...
        // Debug: show what R1 and R2 contain
        console.log("[DEBUG] R1 vertices:", R1.vertices.map(v => v.id));
        console.log("[DEBUG] R1 edges:", R1.edges.map(e => `${e.from.id}→${e.to.id} C=${e.constraint||'ε'} L=${e.maxTraversals}`));
        if (R2) {
            console.log("[DEBUG] R2 vertices:", R2.vertices.map(v => v.id));
            console.log("[DEBUG] R2 edges:", R2.edges.map(e => `${e.from.id}→${e.to.id} C=${e.constraint||'ε'} L=${e.maxTraversals}`));
        } else {
            console.log("[DEBUG] R2: null (no RBS)");
        }
        ...
        if (casSet.length === 0) {
            ...
            console.log("Single CAS found – proper termination satisfied.");
            properTerminationWeakened = true;
            generalizedImpedance      = null;  
            impedanceResult = {
                pass: true,
                sharedArcs: [],
                violations: [],
                message: "Only one CAS exists – generalized impedance is not applicable"
            };
        ...
        const criteria = [
            ...
            },
            {
                ...
                description:
                    generalizedImpedance === null
                        ? "Generalized Impedance: Not Applicable"
