...
//   - Conversion between TLDR Graph model ↔ flat RDLT model
...
// ======================================================================
// SECTION 2: TLDR ↔ FLAT CONVERSION
// ======================================================================

/**
 * Convert a TLDR Graph into the flat RDLT format used by matrix MCA.
 *
 * TLDR edges store Vertex objects at .from / .to, constraint strings,
 * and maxTraversals (L-values).  The flat model uses string vuid references.
 *
 * @param {Graph} tldrGraph - TLDR Graph object (from EVSA R1 or R2).
 * @returns {{ flatRDLT: Object, vertexLookup: Map, edgeLookup: Map }}
 *   flatRDLT   – the converted graph in flat format
 *   vertexLookup – Map<vuid, tldrVertex> for back-conversion
 *   edgeLookup   – Map<"from->to", tldrEdge> for back-conversion
 */
export function graphToFlat(tldrGraph) {
    const vertexLookup = new Map();
    const edgeLookup = new Map();

    // Build vertices – use vertex.id as vuid
    const flatVertices = tldrGraph.vertices.map((v, idx) => {
    ...
    const flatArcs = tldrGraph.edges.map((e, idx) => {
        const key = `${e.from.id}->${e.to.id}`;
        edgeLookup.set(key, e);

        // Map constraint: TLDR uses '' or a string; flat uses 'E' for epsilon
...
/**
 * Convert a flat MAS result back into a TLDR Graph.
 *
 * @param {Object} flatMAS - Flat RDLT representing a MAS.
 * @param {Map} vertexLookup - Map<vuid, tldrVertex> from the original conversion.
 * @param {Map} edgeLookup - Map<"from->to", tldrEdge> from the original conversion.
 * @param {Graph} sourceGraph - The original TLDR Graph (R1 or R2) for edge reference.
 * @returns {Graph} A TLDR Graph representing the MAS.
 */
export function flatToGraph(flatMAS, vertexLookup, edgeLookup, sourceGraph) {
    ...
    for (const fv of flatMAS.vertices) {
        const tldrVertex = vertexLookup.get(fv.vuid);
        if (tldrVertex) {
            mas.vertices.push(tldrVertex);
        } else {
    ...
    for (const fa of flatMAS.arcs) {
        ...
        const tldrEdge = edgeLookup.get(key);

        if (tldrEdge) {
            // Clone the TLDR edge and update L-value from MAS
            const edgeCopy = { ...tldrEdge };
...
/**
 * Run the complete matrix-based MCA + MAS pipeline on a TLDR Graph.
 *
 * This is the primary entry point. Given a TLDR Graph (R1 or R2 from EVSA),
 * it produces an array of MAS as TLDR Graph objects — drop-in compatible
 * with what MASExtractor.extractAllMAS() previously returned.
 *
 * @param {Graph} tldrGraph - TLDR Graph (vertex-simplified R1 or R2).
 * @param {Vertex} source - Source vertex.
 * @param {Vertex} sink - Sink vertex.
 * @returns {Array<Graph>} Array of MAS, each as a TLDR Graph.
 */
export function extractMASviaMatrixMCA(tldrGraph, source, sink) {
    console.log(`[MatrixMCA] === Starting Matrix MCA Pipeline ===`);
    console.log(`[MatrixMCA] Source: ${source.id}, Sink: ${sink.id}`);
    console.log(`[MatrixMCA] Graph: ${tldrGraph.vertices.length} vertices, ${tldrGraph.edges.length} edges`);

    // Step 1: Convert to flat model
    const { flatRDLT, vertexLookup, edgeLookup } = graphToFlat(tldrGraph);
    ...
    // Step 5: Convert back to TLDR Graph objects
    const TLDRMAS = uniqueFlatMAS.map(flatMAS =>
        flatToGraph(flatMAS, vertexLookup, edgeLookup, tldrGraph)
    );

    console.log(`[MatrixMCA] === Pipeline Complete: ${TLDRMAS.length} MAS ===`);
    return TLDRMAS;
}
 ...
 * Works with both TLDR Graph objects and plain {vertices, edges} objects.
 */
